...
res1 = read.csv(paste0(deseq_outDir, "deseq_results/res_ControlvsOutsideDegron.csv"), header = T)
...
res2 = read.csv(paste0(deseq_outDir, "deseq_results/res_ControlvsWithin_degronSGS.csv"), header = T)
...
res3 = read.csv(paste0(deseq_outDir, "deseq_results/res_ControlvsTruncating.csv"), header = T)
...
#res4 = read.csv(paste0(deseq_outDir, "deseq_results/res_ControlvsOutside.csv"), header = T)
...
res5 = read.csv(paste0(deseq_outDir, "deseq_results/res_Outside_degronvsWithin_degronSGS.csv"), header = T)
...
res6 = read.csv(paste0(deseq_outDir, "deseq_results/res_within_degronSGSvstruncating.csv"), header = T)
...
res7 = read.csv(paste0(deseq_outDir, "deseq_results/res_Outside_degronvstruncating.csv"), header = T)
...
for (i in 1:7) {
  ...
  #saveRDS(tg.BP, paste0(deseq_outDir, "GOtermAnalysis/GO_BP_upRegulated_", i, ".rds"))
  tg.BP = readRDS(paste0(deseq_outDir, "GOtermAnalysis/GO_BP_upRegulated_", i, ".rds"))
  ...
  fishtab_BP = GenTable(
    ...
    Fisher = fisher.res,
    orderBy = "Fisher",
    topNodes = geneData(fisher.res)[4]
  )
  # 
  ...
  #saveRDS(tg.CC, paste0(deseq_outDir, "GOtermAnalysis/GO_CC_upRegulated_", i, ".rds"))
  tg.CC = readRDS(paste0(deseq_outDir, "GOtermAnalysis/GO_CC_upRegulated_", i, ".rds"))
  ...
  #saveRDS(tg.MF, paste0(deseq_outDir, "GOtermAnalysis/GO_MF_upRegulated_", i, ".rds"))
  tg.MF = readRDS(paste0(deseq_outDir, "GOtermAnalysis/GO_MF_upRegulated_", i, ".rds"))
  ...
  for (j in c("BP", "CC", "MF")) {
    ...
    tmp_annotation = lapply(get(paste0("allGO_", j)), function(x) x[x %in% names(tmp_condition_up)])
      ...
      for (l in 1:10) {
        ...
        write.table(geneIDs, 
                    paste0(deseq_outDir, "GOtermAnalysis/gene_lists/GO_geneList_", j, "_",
...
ID_genes = read.csv("/data/workspaces/lag/workspaces/lg-scrna/working_data/gokberk/setbp1/ID_whole_panelapp.csv", header = F)
ID_genes = ID_genes$V1
autism_genes = read.csv("/data/workspaces/lag/workspaces/lg-scrna/working_data/gokberk/setbp1/autism_whole_panelapp.csv", header = F)
autism_genes = autism_genes$V1
...
for (i in deseq_files) {
  ...
  if (any(setbp1_targets %in% tmp_f$symbol)) {
    ...
    print("no overlap for setbp1 targets")
    
}

# No overlap for setbp1 targets
...
for (i in deseq_files) {
  ...
  if (any(ID_genes %in% tmp_f$symbol)) {
    
    write.csv(intersect(ID_genes, tmp_f$symbol), 
              file = paste0(deseq_outDir, "deseq_results/ID_genes_overlap_", tmp_f_name),
              quote = F, row.names = F)
    
  }
  else
    print("no overlap for ID_genes")
...
for (i in deseq_files) {
  ...
  if (any(autism_genes %in% tmp_f$symbol)) {
    
    write.csv(intersect(autism_genes, tmp_f$symbol), 
              file = paste0(deseq_outDir, "deseq_results/autism_genes_overlap_", tmp_f_name),
              quote = F, row.names = F)
    
  }
  else
    print("no overlap for autism_genes")
...
for (i in 1:7) {
  ...
  tmp_subset = data.frame(gene = rownames(tmp_dds),
                          tmp_dds,
                          stringsAsFactors = F) %>%
                          na.omit() %>%
                          filter(padj<0.1) %>%
  ...
  top_6_DEGs = list(head(tmp_dds$symbol))[[1]]
  goi2 = c(goi, top_6_DEGs)
  
  for (j in goi2) {
    
    if (is.na(j)) next
    if (!any(tmp_subset$symbol == j)) next
    
    else
      
      pdf(file = paste0(deseq_outDir, "boxplots/res", i, "_", j, ".pdf"), width = 12, height = 8)
      plotCounts(dds, gene = tmp_subset$geneIDs[tmp_subset$symbol == j],
                 intgroup="condition", main = j)
      dev.off()
...
# make lists for RRHO and save them as csv files
for (i in 1:7) {
  if (i==4) next
  ...
  write.csv(tmp_list, 
            file = paste0(deseq_outDir, "RRHO_sigDEgenes/list_", i, ".csv"),
            quote = F, row.names = F)
  
}

# run RRHO for each list pair
  for (j in  1:7) {
    if (j==4) next
    
    tmp_list1 = read.csv(paste0(deseq_outDir, "RRHO_sigDEgenes/list_", j, ".csv"))
    ...
    for (k in  1:7) {
      ...
      tmp_list2 = read.csv(paste0(deseq_outDir, "RRHO_sigDEgenes/list_", k, ".csv"))
      ...
      RRHO(tmp_list1, tmp_list2, stepsize = 100, labels = (c(as.character(j),as.character(k))), alternative = "enrichment", #100 us recommended as the step size
           plots = T, outputdir = paste0(deseq_outDir, "RRHO"), BY = T, log10.ind = T)
